...
// debug tracing -> Nuke terminal (stderr). Toggle with CUTIE_DEBUG env or always on.
#define CDBG(...) do { std::cerr << "[CutieRoto] "; std::fprintf(stderr, __VA_ARGS__); \
...
void CutieRoto::buildPipeline() {
    ...
    e1_ = std::make_unique<cutie::TRTEngine>(cutie::asset_e1().data, cutie::asset_e1().size, "encode_image", runtime_.get());
    e2_ = std::make_unique<cutie::TRTEngine>(cutie::asset_e2().data, cutie::asset_e2().size, "transform_key", runtime_.get());
    e3_ = std::make_unique<cutie::TRTEngine>(cutie::asset_e3().data, cutie::asset_e3().size, "mask_encoder", runtime_.get());
    e5_ = std::make_unique<cutie::TRTEngine>(cutie::asset_e5().data, cutie::asset_e5().size, "mask_decoder", runtime_.get());
...
std::string CutieRoto::cacheDir() const {
    ...
    if (d.empty()) {
        ...
        d = "/tmp/cutieroto/" + (nm.empty() ? std::string("CutieRoto") : nm);
    }
    return d;
}
...
void CutieRoto::processAllFrames() {
    ...
    for (auto& kf : kfs) {
        ...
        int fr = kf.second;
        torch::Tensor img  = pullInputResized(0, fr, false);
        torch::Tensor roto = pullInputResized(ROTO_INPUT, fr, true);
    ...
    for (int fr = first; fr <= last; ++fr) {
        ...
        if (kfIt != kfAtFrame.end() && input(kfIt->second)) {
            // this frame IS a keyframe: pin output to the artist roto (it overrides
            // the prediction), and refresh last_mask/sensory from it.
            torch::Tensor roto = pullInputResized(kfIt->second, fr, true);
            if (roto.defined()) {
                alphaPad = roto;
                lastMask_ = roto;
                CDBG("frame %d is keyframe -> using artist roto", fr);
            }
        }
        if (!alphaPad.defined()) {
        ...
        {
            std::lock_guard<std::mutex> lk(cacheMutex_);
            alphaCache_[fr] = std::move(a);
            cacheW_ = pw; cacheH_ = ph;
        }
        ++done;
...
void CutieRoto::engine(int y, int x, int r, ChannelMask m, Row& row) {
    ...
    const std::vector<float>* cached = nullptr;
    int cw = 0, ch = 0;
    {
        std::lock_guard<std::mutex> lk(cacheMutex_);
        auto it = alphaCache_.find(frame);
        if (it != alphaCache_.end()) { cached = &it->second; cw = cacheW_; ch = cacheH_; }
    }
    ...
    if (!cached) {
        ...
        if (readRawMatte(frame, a, w, h)) {
            std::lock_guard<std::mutex> lk(cacheMutex_);
            alphaCache_[frame] = std::move(a);
            cacheW_ = w; cacheH_ = h;
            cached = &alphaCache_[frame]; cw = w; ch = h;
        }
    }
    // trace once per engine pass (only on the first scanline to avoid spam)
    static int lastLoggedFrame = -99999;
    if (y == fY_ && frame != lastLoggedFrame) {
        lastLoggedFrame = frame;
        CDBG("engine: frame=%d cached=%d cw=%d ch=%d fW=%d fH=%d",
    ...
    if (cached && cw == fW_ && ch == fH_) {
        const int dy = (fY_ + fH_ - 1) - y;   // flip
        if (dy >= 0 && dy < fH_) {
            const float* srcRow = cached->data() + (size_t)dy * fW_;
            for (int xi = x; xi < r; ++xi) {
                int sx = xi - fX_;
                float v = (sx >= 0 && sx < fW_) ? srcRow[sx] : 0.f;
                outA[xi - x] = post(v);
            }
        } else {
