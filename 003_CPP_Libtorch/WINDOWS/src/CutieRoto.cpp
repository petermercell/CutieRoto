...
void CutieRoto::engine(int y, int x, int r, ChannelMask m, Row& row) {
    ...
    CDBG("engine ENTER: frame=%d y=%d x=%d r=%d", frame, y, x, r);
    ...
    if (input(0)) {
        CDBG("engine[y=%d x=%d r=%d]: input0->get", y, x, r);
    ...
    CDBG("engine[y=%d]: get writable Chan_Alpha (x=%d r=%d)", y, x, r);
    float* outA = row.writable(Chan_Alpha) + x;
    CDBG("engine[y=%d]: outA=%p (r-x=%d)", y, (void*)outA, r - x);
    ...
    if (cached && cw > 0 && ch > 0 && cached->a.size() == (size_t)cw * (size_t)ch
        && fW_ > 0 && fH_ > 0) {
        CDBG("engine[y=%d]: SERVE cached size=%zu cw=%d ch=%d fX=%d fY=%d fW=%d fH=%d",
        ...
        if (py >= 0 && py < fH_) {
            ...
            if (sy < 0) sy = 0;
            if (sy >= ch) sy = ch - 1;
            CDBG("engine[y=%d]: py=%d dyTop=%d sy=%d srcRow=cached+%zu", y, py, dyTop, sy, (size_t)sy*cw);
            ...
            CDBG("engine[y=%d]: serve loop done", y);
        } else {
