...
// debug tracing -> Nuke terminal (stderr). OFF by default; enable by launching
// Nuke with  CUTIE_DEBUG=1  in the environment. Checked once and cached so the
// per-frame engine() calls don't flood the terminal (which also slows scrubbing).
namespace { inline bool cutieDebugOn() {
